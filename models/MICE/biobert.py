...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default='dmis-lab/biobert-base-cased-v1.2', metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )
...
def get_biobert(model_name='dmis-lab/biobert-base-cased-v1.2'):
    # We now keep distinct sets of args, for a cleaner separation of concerns.
    #parser = HfArgumentParser((ModelArguments, EmbeddingArguments))
    ...
    #model = AutoModel.from_pretrained(model_name, from_tf=False, config=config, cache_dir=None, ignore_mismatched_sizes=True)
