...
class MOEBlock(nn.Module):
    def __init__(self, final_embed_dim, num_task, kernel_size=1, with_feat=False, rank_list=[8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120]):
        ...
        #self.share_conv = nn.Linear(final_embed_dim, final_embed_dim)
        #self.bn = nn.ModuleList()
        self.bn_all = nn.ModuleList()
        #self.activate = nn.GELU()
        #self.task_att = nn.Parameter(torch.randn(num_task, 1, final_embed_dim), requires_grad=True)
        for idx in range(num_task):
            #self.conv1.append(nn.Linear(final_embed_dim, final_embed_dim))
            #self.conv3.append(nn.Linear(final_embed_dim, final_embed_dim))
            self.conv2.append(LoraBlock_trans(final_embed_dim, kernel_size=kernel_size, rank=64))

            #self.bn.append(nn.LayerNorm(final_embed_dim))
        ...
        for idx in range(num_task):
            ...
            #self.router_1[str(task)].append(SpatialAtt(final_embed_dim, final_embed_dim // 4, im_size=final_embed_dim, with_feat=with_feat))
            #self.router_1[str(task)].append(nn.Linear(final_embed_dim // 2, self.num_lora * 2 + 1))
    ...
    def forward(self, x, task, route_feat_in=None, phase='train'):
        ...
        #print(route_1_raw.shape, stdev_1.shape, prob_mix.shape)
        if phase == 'train':
            noise = torch.randn_like(route_1_raw) * stdev_1
        else:
            noise = 0
        #print(route_1_raw, noise)
        if self.pre_softmax:
            ...
            #print(route_1_raw.shape, noise.shape)
            route_1_indice = torch.topk(route_1_raw, self.desert_k, dim=1, largest=False)[1]
        ...
        #print(lora_out_1.shape)
        lora_out_1 = torch.sum(lora_out_1 * route_1, dim=1)
        #print(out.shape, self.conv2[task](out).shape, prob_mix[:, 0].shape)
        moe_out = self.bn_all[task](lora_out_1)
        spec_out = self.conv2[task](xfeat) * prob_mix[:, 0]
        share_out = self.share_conv(xfeat)
        #out = moe_out + spec_out + share_out
        #out = self.bn[task](out)
        #out = self.activate(out)
        #out = self.conv3[task](out)
        #out = torch.sigmoid(self.task_att[task]) * out
        #print(moe_out.shape, spec_out[:, 0].shape, share_out[:, 0].shape)
